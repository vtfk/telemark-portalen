...
function validateIp(options) {
  ...
  const include =
    shortcut.includeIps && Array.isArray(shortcut.includeIps)
      ? shortcut.includeIps.filter(ip => wildcard(ip, myIp)).length > 0
      : true
  const exclude =
    shortcut.excludeIps && Array.isArray(shortcut.excludeIps)
      ? shortcut.excludeIps.filter(ip => wildcard(ip, myIp)).length !== 0
      : false
...
function ipFilter(options) {
  const shortcuts = options.shortcuts.map(shortcut =>
    Object.assign(shortcut, {
      isValid: validateIp({ shortcut: shortcut, myIp: options.myIp })
    })
  )
...
module.exports = data => {
  ...
  const finalRoles = makeUnique(
    myIp ? ipFilter({ shortcuts: myShortcuts, myIp: myIp }) : myShortcuts
  )
  return finalRoles
}
